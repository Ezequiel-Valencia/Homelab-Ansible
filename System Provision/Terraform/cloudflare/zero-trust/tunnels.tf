...
resource "cloudflare_zero_trust_tunnel_cloudflared_config" "homelab_tunnel_config" {
  ...
  config = {
    ...
    {
        hostname = "seer.tunnel.homelab.ezequielvalencia.com"
        service  = "http://seer.media.svc.cluster.local:80"
        origin_request = {
        no_happy_eyeballs = true 
      }
    },
    {
        hostname = "rom.tunnel.homelab.ezequielvalencia.com"
        service  = "http://romm.media.svc.cluster.local:80"
        origin_request = {
        no_happy_eyeballs = true 
      }
    },
